...
describe('BinarySearchTree', function() {
  ...
  context('Remove', function () {
      ...
      it('should throw an error if no value provided', () => {
          expect(() => {this.bst.remove()}).to.Throw(TypeError,'remove() expects a value to be provided.')
      })

      it('should return error if value not in tree', () => {
          expect(() => {this.bst.remove(55)}).to.Throw(RangeError, "Value is not in tree")
      })

      it('should return error if empty tree', () => {
          expect(() => {new BinarySearchTree().remove(55)}).to.Throw(RangeError, "Value is not in tree")
      })
