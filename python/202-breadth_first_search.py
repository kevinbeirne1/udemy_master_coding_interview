...
class BinarySearchTree:
    ...
    def breadth_first_search_recursive(self, node_queue=None, visited=None):
        if not self.root:
            return []
        if not visited:
            node_queue = deque([self.root])
        ...
        if current.right:
            node_queue.append(current.right)
        return self.breadth_first_search_recursive(node_queue, visited)
...
class TestBreadthFirstSearch(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        cls.test_cases = [
            ...
            ([9, 4, 6, 20, 170, 15, 1], [9, 4, 20, 1, 6, 15, 170]),
        ]

    def test_breadth_first_search_iterative(self):
        for test_case, expected_result in self.test_cases:
            with self.subTest(f"Test case: {test_case}"):
                tree = BinarySearchTree()
                tree.populate_tree(test_case)
                actual = tree.breadth_first_search_iterative()
                self.assertEqual(actual, expected_result)

    def test_breadth_first_search_recursive(self):
        for test_case, expected_result in self.test_cases:
            with self.subTest(f"Test case: {test_case}"):
                tree = BinarySearchTree()
                tree.populate_tree(test_case)
                actual = tree.breadth_first_search_recursive()
                self.assertEqual(actual, expected_result)
